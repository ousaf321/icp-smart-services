(function (angular) {
    ...
    var Selector = (function () {
        ...
        function Selector(filter, timeout, window, http, q) {
            ...
            this.scope = {
                ...
                qsValidation: '@?', // add for validation (TAHALUF)
                tName: '@?'
            };
            ...
            transclude(scope, function (clone, scope) {
                ...
                scope.fetch = function () {
                    ...
                    if (promise) {
                        if (typeof promise.then !== 'function') {
                            var settings = { method: 'GET', cache: false, params: {} };
                ...
                scope.set = function (option) {
                    ...
                    if (!scope.multiple) scope.selectedValues = [option];
                        ...
                        var selectedIndex = scope.selectedValues.indexOf(option);
                        if (selectedIndex < 0) {
    ...
    angular
		.module('selector', ['ngSanitize', 'ghiscoding.validation',
    'pascalprecht.translate'])
		.run(['$templateCache', '$translate', 'ValidationService', '$translatePartialLoader', function ($templateCache, $translate, ValidationService, $translatePartialLoader) {
				...
				'<div class="selector-container" ng-attr-dir="{{rtl ? \'rtl\' : \'ltr\'}}" ' +
					...
					'<label class="selector-input">' +
						'<ul class="selector-values">' +
							'<li ng-repeat="(index, option) in selectedValues | limitTo: 4 track by index">' +
								'<div ng-include="viewItemTemplate"></div>' +
								'<div ng-if="multiple" class="selector-helper" ng-click="!disabled && unset(index)">' +
									'<span class="selector-icon"></span>' +
								'</div>' +
							'</li>' +
							'<li ng-if="selectedValues.length > 4" class="selector-overflow flex-shrink-0">+{{ selectedValues.length - 4 }}</li>' +
						'</ul>' +
					...
					'<ul class="selector-dropdown" ng-show="filteredOptions.length > 0 || (create && search)">' +
                        ...
                        '<li ng-repeat-end ng-class="{active: highlighted == index, grouped: option[groupAttr], \'option-selected\': inOptions(selectedValues, option)}" class="selector-option" ' +
    ...
    angular
		.module('selector').config(['$translateProvider', '$translatePartialLoaderProvider', '$animateProvider',
    function ($translateProvider, $translatePartialLoaderProvider, $animateProvider) {
