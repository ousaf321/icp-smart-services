...
.controller('UibYearpickerController', ['$scope', '$element', 'dateFilter', function (scope, $element, dateFilter) {
    ...
    this._refreshView = function () {
        ...
        //moayad 
        //fix show disabled date on click
        //12/12/2016

        var showingDate = {};
        if (this.maxDate) {
            if (this.maxDate < this.activeDate) {
                showingDate = this.maxDate;
                //  this.activeDate = showingDate;
            }
            else {
                showingDate = this.activeDate;
            }
        }
        else
            showingDate = this.activeDate;

        for (var i = 0, start = getStartingYear(showingDate.getFullYear()) ; i < range; i++) {
            date = new Date(this.activeDate);
        ...
        if (value) {
            if (!$scope.disabled) {
                $timeout(function () {
                    ...
                    //Translate calendar buttons :: Start [Tenh]
                    var els = document.getElementsByClassName('btn btn-sm btn-danger uib-clear ng-binding');
                    for (var i = 0; i < els.length; i++) {
                        els[i].innerText = $locale.clearText;
                    }
                    els = document.getElementsByClassName('btn btn-sm btn-success pull-right uib-close ng-binding');
                    for (var i = 0; i < els.length; i++) {
                        els[i].innerText = $locale.closeText;
                    }
                    els = document.getElementsByClassName('btn btn-sm btn-info uib-datepicker-current ng-binding');
                    for (var i = 0; i < els.length; i++) {
                        els[i].innerText = $locale.todayText;
                    }


                    //Translate calendar buttons :: End [Tenh]
...
.directive('uibDatepickerPopup', function () {
    ...
    return {
        ...
        link: function (scope, element, attrs, ctrls) {
            ...
            ctrl.init(ngModel);
            //handel min and max when writing date manually/prevent user to write invalid value :: Start [Tenh]
            element.bind('blur', function (evt) {
                ...
                if (scope.date == undefined || scope.date == '') {
                    ...
                    if (scope.watchData.minDate) {
                        var min = scope.watchData.minDate;
                        if (scope.watchData.minDate instanceof Date) {
                            min = moment(scope.watchData.minDate).format('YYYY/MM/DD');
                        }

                        var momentDate = moment(min).startOf('day');
                        var minDate = new Date(momentDate.year() + '/' + (momentDate.month() + 1) + '/' + momentDate.date());
                        if (new Date(moment(scope.date).format('YYYY/MM/DD')) < minDate) {
                            element.val(null);
                            ngModel.$setViewValue(null);
                        }
                    }
                    if (scope.watchData.maxDate) {
                        var max = scope.watchData.maxDate;
                        if (scope.watchData.maxDate instanceof Date) {
                            max = moment(scope.watchData.maxDate).format('YYYY/MM/DD');
                        }
                        var momentDate = moment(max).startOf('day');
                        var maxDate = new Date(momentDate.year() + '/' + (momentDate.month() + 1) + '/' + momentDate.date());
                        if (new Date(moment(scope.date).format('YYYY/MM/DD')) > maxDate) {
                            element.val(null);
                            ngModel.$setViewValue(null);
                        }
                    }
            ...
            //from model to view
            //value should be string like this [2014/09/13] or comes from server as utc string [2014-09-13T00:00:00Z] or
            ...
            //moment.utc('2014-09-13T00:00:00Z').format() "2014-09-13T00:00:00Z"
            //moment('2014-09-13T00:00:00Z').format() "2014-09-13T03:00:00+03:00"
            //moment.utc('2014/09/13').format() "2014-09-13T00:00:00Z"
            //moment('2014/09/13').format() "2014-09-13T00:00:00+03:00"
            //new date('2014/09/13') Fri May 16 2014 00:00:00 GMT+0200
            ngModel.$formatters.push(function (value) {
                if (!value) return "";
                return new Date(value);
                //var utcDate = moment.utc(value).startOf('day');
                //var date = new Date(utcDate.year() + '/' + (utcDate.month() + 1) + '/' + utcDate.date());
                //return date;
            ...
            //handel min and max when writing date manually/prevent user to write invalid value :: End
        }
    };
})
