...
(function (app) {
    ...
    app.factory('uiTourBackdrop', ['TourConfig', '$document', '$uibPosition', '$window', function (TourConfig, $document, $uibPosition, $window) {
        ...
        function positionBackdrop(element, isFixedElement) {
            ...
            viewWindow.top.css({
                position: isFixedElement ? 'fixed' : 'fixed',
                top: 0,
                left: 0,
                width: '100%',
                height: '100%'
...
angular.module('bm.uiTour').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put("tour-step-template.html",
    ...
    "        <div class=\"btn-group\">\n" +
    "            <button class=\"btn btn-sm btn-default\" ng-if=\"tourStep.isPrev()\" ng-click=\"tour.prev()\">&laquo; {{'Prev' | translate}}</button>\n" +
    "            <button class=\"btn btn-sm btn-default pull-right\" ng-if=\"tourStep.isNext()\" ng-click=\"tour.next()\">{{'Next' | translate}} &raquo;</button>\n" +
     "        <button class=\"btn btn-sm btn-danger pull-right\" ng-if=\"!tourStep.isNext()\" data-role=\"end\" ng-click=\"tour.end()\">{{'endTour'|translate}}</button>\n" +
    "            <button class=\"btn btn-sm btn-default btn-gray\" data-role=\"pause-resume\" data-pause-text=\"{{'endTour'|translate}}\"\n" +
    "                    data-resume-text=\"Resume\" ng-click=\"tour.pause()\">{{'endTour'|translate}}\n" +
    "            </button>\n" +
    "        </div>\n" +
    "    </div>\n" +
    "</div>\n" +
